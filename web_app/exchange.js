...
const exchange_name = "swapp"; // TODO: fill in the name of your exchange

const token_name = "bestCoin"; // TODO: replace with name of your token
const token_symbol = "HNN"; // TODO: replace with symbol for your token
...
async function addLiquidity(amountEth, maxSlippagePct) {
    ...
    await token_contract
        ...
        .approve(
            exchange_address,
            Math.floor(rates.max_exchange_rate * amountEth)
        );
    await exchange_contract
        .connect(provider.getSigner(defaultAccount))
...
provider.listAccounts().then((response) => {
    ...
    $(".account").html(opts);
    getAccount(response[0]).then((res) => {
        $("#account_detail").html(`
            <p> HNN: ${res["start_tokens"]} </p>
        `);
    });
});
...
$("#myaccount").on("change", function () {
    // alert( this.value );
    getAccount(this.value).then((res) => {
        $("#account_detail").html(`
          <p> HNN: ${res["start_tokens"]} </p>
        `);
    });
...
async function getAccount(defaultAccount) {
    var start_tokens = await token_contract
        .connect(provider.getSigner(defaultAccount))
        .balanceOf(defaultAccount);
    return {
        start_tokens: Number(start_tokens),
    };
}
...
const sanityCheck = async function () {
    ...
    if (Number(swap_fee[0]) == 0) {
        ...
        score += check(
            ...
            state2.token_liquidity === state1.token_liquidity + 100 &&
                Math.abs(
                    state1.eth_liquidity -
                        expected_eth_received -
                        state2.eth_liquidity
                ) < 5 &&
        ...
        score += check(
            ...
            state3.eth_liquidity === state2.eth_liquidity + 100 &&
                ...
                Math.abs(
                    Number(user_tokens3) -
                        (Number(user_tokens2) - expected_tokens_added)
                ) < 5
        );
    ...
    // LP provider rewards implemented
        ...
        await swapTokensForETH(100, 1);
        var state2 = await getPoolState();
        var expected_eth_received =
            100 * (1 - swap_fee) * state1.eth_token_rate;
        var user_tokens2 = await token_contract
            .connect(provider.getSigner(defaultAccount))
            .balanceOf(defaultAccount);
        score += check(
